...
# --- 앱 시작 시 메모 데이터 로드 (st.session_state 초기화 부분에 추가) ---
if 'memos' not in st.session_state:
    st.session_state['memos'] = load_memos()

# --- 메모 앱 기능 함수 (아직 통합 전이라 임시로) ---
def memo_feature_section():
    st.header("🗒️ 나만의 메모장")

    memo_content = st.text_area("새로운 메모를 입력하세요:", key="new_memo_input_ch4")
    is_starred = st.checkbox("⭐ 즐겨찾기로 등록", key="star_checkbox_ch4")

    if st.button("메모 저장", key="save_memo_button_ch4"):
        if memo_content:
            st.session_state['memos'].append({"text": memo_content, "star": is_starred})
            save_memos(st.session_state['memos']) # 저장 함수 호출
            st.success("메모가 성공적으로 저장되었습니다!")
            st.session_state["new_memo_input_ch4"] = "" # 입력창 초기화
        else:
    ...
    # 검색 및 필터링 기능
    search_query = st.text_input("메모 검색 (내용에 포함된 단어)", key="search_memo_input_ch4")
    filter_option = st.selectbox(
        "메모 필터",
        ("전체 메모", "⭐ 즐겨찾기 메모"),
        key="filter_memo_select_ch4"
    )

    filtered_memos = []
    for memo in st.session_state['memos']:
        search_match = search_query.lower() in memo['text'].lower() if search_query else True
        if filter_option == "⭐ 즐겨찾기 메모":
    ...
    if filtered_memos:
        for i, memo in enumerate(filtered_memos):
            star_icon = "⭐ " if memo['star'] else ""
            st.write(f"- {star_icon}{memo['text']}")
    else:
        st.info("조건에 맞는 메모가 없습니다.")

# (현재는 app.py 하단에 임시로 호출하거나 주석 처리)
